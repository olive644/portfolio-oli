    ...
    <section id="projetos" className="border-b border-border py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <Reveal className="grid gap-6 sm:grid-cols-[180px_1fr]">
          ...
          <div>
            <h2 className="max-w-3xl text-5xl leading-none sm:text-7xl">Abra a pasta. Veja o que já construí.</h2>
            <p className="mt-5 max-w-xl text-muted-foreground">Clique para espalhar os projetos. Arraste uma imagem para baixo para guardar tudo novamente.</p>
          </div>
        </Reveal>

        <Reveal className="mt-4" delay={100}>
          <InteractiveFolderGallery photos={galleryPhotos} folderName="Meus projetos.folder" dragHintText="Arraste uma imagem para baixo para fechar" />
        </Reveal>

        <div className="divide-y divide-border border-y border-border">
          {projects.map((project) => (
            <Reveal key={project.number}>
              <a href={project.href} target="_blank" rel="noreferrer" className="group grid gap-4 py-6 sm:grid-cols-[64px_1fr_1fr_auto] sm:items-center">
                <span className="font-mono text-xs text-muted-foreground">{project.number}</span>
                <div><h3 className="text-3xl leading-none">{project.title}</h3><p className="mt-2 font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground">{project.kind}</p></div>
