...
const schema = z.object({
  ...
  email: z.string().trim().email("Email em formato inválido.").max(255),
...
export function Contact() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    } catch { toast.error("Não foi possível enviar.", { description: "Tente novamente ou use o email ao lado." }); }
      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <Reveal className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr]">
          <div>
            ...
            <h2 className="mt-6 text-6xl leading-[0.9] sm:text-8xl">Tem algo em mente?</h2>
          ...
          <form onSubmit={submit} noValidate className="border-t border-border pt-3">
            {(["name", "email"] as const).map((key) => <label key={key} className="block border-b border-border py-5"><span className="font-mono text-xs uppercase text-muted-foreground">{key === "name" ? "Nome" : "Email"}</span><input type={key === "email" ? "email" : "text"} value={form[key]} onChange={(e) => setValue(key, e.target.value)} placeholder={key === "name" ? "Como você se chama?" : "voce@email.com"} className={field} />{errors[key] && <span className="mt-2 block text-xs text-destructive">{errors[key]}</span>}</label>)}
            <label className="block border-b border-border py-5"><span className="font-mono text-xs uppercase text-muted-foreground">Mensagem</span><textarea rows={5} value={form.message} onChange={(e) => setValue("message", e.target.value)} placeholder="Conte um pouco sobre sua ideia" className={`${field} resize-none`} />{errors.message && <span className="mt-2 block text-xs text-destructive">{errors.message}</span>}</label>
