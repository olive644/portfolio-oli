  ...
  return (
    <div className={`relative w-full py-12 sm:py-24 ${className || ""}`}>
      <div className="relative flex min-h-[520px] w-full flex-col items-center justify-center sm:min-h-[650px]">
        <div className="pointer-events-none relative flex h-[500px] w-full max-w-[760px] justify-center sm:h-[580px]">
          <motion.div
                ...
                <motion.div
                  ...
                  animate={
                    !isFolderOpen
                      ...
                      : { y: isMobile ? -145 : -165, x: openX, rotate: 0, scale: isMobile ? 0.88 : 1.02, zIndex: 50 }
                  }
                  whileHover={isFolderOpen ? { scale: isMobile ? 0.92 : 1.07, zIndex: 100 } : {}}
                  whileDrag={isFolderOpen ? { scale: isMobile ? 0.96 : 1.1, rotate: 4, zIndex: 150 } : {}}
                  transition={{ type: "spring", stiffness: 350, damping: 30 }}
                >
                  <img src={photo.image} alt={photo.title || "Item da galeria"} className="pointer-events-none h-full w-full object-cover" />
                  {photo.title && (
                    <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-linear-to-t from-black via-black/80 to-transparent p-4 pt-12">
                      <p className="font-display text-xl leading-none text-white">{photo.title}</p>
                      {photo.href && (
